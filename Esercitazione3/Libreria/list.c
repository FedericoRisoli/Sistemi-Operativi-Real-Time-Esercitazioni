...
/* Confronta due elementi della lista:
   ...
   - ritorna un valore <0 se item1 < item2;
   - ritorna un valore ==0 se item1 == item2.
  NB: puo' essere utilizzata nelle funzioni di ricerca e/o ordititolonto
*/
...
LIST EnqueueFirst(LIST l, ItemType item) {
        ...
        NODE* new_node =createNode(item); //creo un nuovo nodo
        ...
        new_node->next=l; //faccio puntare la nuova testa a quella vecchia
        l=new_node; //il nuovo nodo diventa la nuova testa
...
/* Decrementa il numero di copie di un libro e rimuove i libri con quantità 0*/
LIST DecreaseCopy(LIST l, ItemType item) {
    ItemType*current=Find(l,item);
    NODE* tmp=l;

    if(current!=NULL && current->num_copie>0){
      current->num_copie-=1;
    }
    //gestisco testa
    while(l!=NULL && l->item.num_copie==0)
    {
      NODE* todel=l;
      printf("Rimuovo dalla lista %s, possibile rifornire\n", l->item.titolo);
      l=l->next;
      free(todel);
    }

    while(tmp != NULL && tmp->next!=NULL){
      if(tmp->next->item.num_copie==0){
        NODE* todel=tmp->next;
        printf("Rimuovo dalla lista %s, possibile rifornire\n", todel->item.titolo);
...
LIST EnqueueOrdered(LIST l, ItemType item) {
  ...
  //se la lista è vuota o il nuovo elemento è più piccolo della testa
   if (isEmpty(l)||itemCompare(item, l->item)<0) {
    new_node->next=l; //il nuovo nodo diventa la testa
      return new_node;
      }else{
      LIST tmp=l;
      //scorro la lista fino a trovare il punto di inserimento
  while (!isEmpty(tmp->next) && itemCompare(new_node->item, tmp->next->item)>=0) {
  tmp=tmp->next;
  }

  //inserisco il nuovo nodo tra tmp e tmp->next
...
void PrintItem(ItemType item) {
    printf("\nTitolo: %s, Numero Copie: %.d\n", item.titolo, item.num_copie);
}
