...
LIST DequeueLast(LIST l) {
    if (!isEmpty(l)) {
        ...
        NODE* todel = l; 
     while (todel->next->next != NULL) //scorro la lista per fermarmi al penultimo nodo
     { 
    	todel = todel->next; 
          
    }
    deleteNode(todel->next); //elimino l'ultimo nodo
    todel->next = NULL; //setto l'  ultimo nodo con puntatore a NULL 
    }
